...
export default function TerbaruPage() {
  ...
  const [comics, setComics] = useState<Comic[]>(latestComicsData);
  ...
  useEffect(() => {
    const fetchLatestComics = async () => {
      ...
      try {
        const response = await fetch(`${API_URL}/terbaru/${pageNumber}`);
    ...
    <Layout>
      <div className="container mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-white">Komik Terbaru</h1>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
            {comics.map((comic, index) => (
              <ComicCard key={`latest-${index}-${comic.link}`} comic={comic} />
            ))}
          </div>
        )}
